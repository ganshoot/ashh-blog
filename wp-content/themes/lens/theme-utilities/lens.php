	...
	static function display_filter_box( $taxonomy_name ){
		...
		if(wpgrade::option('portfolio_projects_filtering') == 1) {
			...
			if(!empty($filter_categories) && !is_wp_error($filter_categories)) { ?>
				<div class="mosaic__filter">
					<div class="filter__btn btn btn--large"><i class="pixcode  pixcode--icon  icon-e-up-circled"></i><span><?php _e('Filter', wpgrade::textdomain()); ?></span>
						<ul>
							<?php foreach($filter_categories as $key=>$category) { ?>
								<li class="mosaic__filter-item" data-filter="<?php echo $category->slug; ?>"><?php echo $category->name; ?></li>
							<?php } ?>
							<li class="mosaic__filter-item" data-filter="all"><?php _e('View All', wpgrade::textdomain()); ?></li>
						</ul>
					</div>
				</div>
			<?php
